...
<head>
	...
	<title>OASMan updater</title>

	<script src="main.js"></script>
	<link rel="stylesheet" href="main.css">
</head>
<body>
	<h1>OASMan OTA updater</h1>
	...
	<p>
		<b>MAX firmware space: </b> %s bytes<br />
		<b>MAC:</b> %s<br />
		<b>Uptime:</b> %s<br />
	</p>
...
int WebOTA::add_http_routes(WebServer *server, const char *path) {
	...
	server->on("/", HTTP_GET, [server]() {
		check_auth(server); // keep your authentication
	...
	server->on(path, HTTP_GET, [server,this]() {
		...
		if (this->custom_html != NULL) {
			...
			char buf[1024];
			snprintf_P(buf, sizeof(buf), INDEX_HTML, (String)maxSketchSpace, mac_addr.c_str(), uptime_str.c_str());
