...
int is_bin_file(const char *filename)
{
    ...
    return (ext != NULL && (strcmp(ext, ".bin") == 0 || strcmp(ext, ".jpg") == 0 || strcmp(ext, ".png") == 0 || strcmp(ext, ".bmp") == 0 || strcmp(ext, ".gif") == 0) );
}
...
static void img_callback(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_SHORT_CLICKED)
    {
        ...
        sprintf(str_buf, "S:/images/%s", bin_filenames[img_index]);
...
void image_tile_init(lv_obj_t *parent)
{
    ...
    read_bin_files("S:/images");
    print_bin_filenames();
    sprintf(str_buf, "S:/images/%s", bin_filenames[0]);
    lv_img_set_src(img, str_buf);
}