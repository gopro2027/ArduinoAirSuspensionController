...
// The GT911 uses 16-bit register addresses, so it needs its own read/write rather than the
// 8-bit helpers in I2C_Driver.cpp.
...
/* Reset controller. Timing follows Waveshare's touch pre-begin hook for this board. */
uint8_t GT911_Touch_Reset(void)
{
  // INT held low across the whole reset window selects I2C address 0x5D.
  pinMode(GT911_INT_PIN, OUTPUT);
  digitalWrite(GT911_INT_PIN, LOW);
  vTaskDelay(pdMS_TO_TICKS(10));

  CH422G_Set_EXIO(EXIO_TP_RST, Low);
  vTaskDelay(pdMS_TO_TICKS(100));

  CH422G_Set_EXIO(EXIO_TP_RST, High);
  vTaskDelay(pdMS_TO_TICKS(200));

  // Release INT so the controller can drive it as the data-ready interrupt.
...
{
  // Zero-initialised, and the status read is checked: on an I2C failure buf[0] would otherwise be
  // stack garbage, and a garbage status byte can look like a real touch (bit7 set + a plausible
  // point count). This UI can command valves, so a fabricated touch is not a cosmetic bug.
  uint8_t buf[41] = {0};
  uint8_t touch_cnt = 0;
  uint8_t clear = 0;
  size_t i = 0;
  if (!I2C_Read_Touch(GT911_ADDR, ESP_LCD_TOUCH_GT911_READ_DATA_REG, buf, 1))
    return false;
  if ((buf[0] & 0x80) == 0x00) {
...
// IRAM_ATTR lives on the declaration in the header; repeating it here makes GCC emit
// "ignoring attribute section(.iram1.1)". The function is still placed in IRAM.
void Touch_GT911_ISR(void)
{
